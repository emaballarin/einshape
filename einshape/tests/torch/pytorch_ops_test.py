...
from einshape.src.torch import torch_ops
from einshape.tests import test_cases
import numpy as np
import torch


class TorchEngineTest(parameterized.TestCase):
  ...
  def test_common(self, x: Sequence[Any], equation: str,
    ...
    x = torch.Tensor(x)
    y = torch_ops.einshape(equation, x, **index_sizes)
    np.testing.assert_array_equal(np.array(expected), y.numpy())

  def test_accepts_python_list(self):
    x = [3, 5]  # Python list, not a JAX tensor.
    y = torch_ops.einshape('i->i1', x)
    np.testing.assert_array_equal(np.array([[3], [5]]), y.numpy())
